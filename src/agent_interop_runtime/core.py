...
class MockAdapter:
    ...
    def capabilities(self) -> AdapterCapabilities:
        return AdapterCapabilities(tools=True)
...
class InteropRuntime:
    ...
    def capabilities(self, adapter_name: str) -> AdapterCapabilities:
        try:
        ...
        return method() if callable(method) else AdapterCapabilities()
    ...
    def run(
        ...
        *,
        ...
        if spec.policy.max_cost_usd is not None:
            raw_cost = result.metadata.get("cost_usd")
            if raw_cost is not None and float(raw_cost) > spec.policy.max_cost_usd:
                raise RuntimeError(
                    f"adapter exceeded max_cost_usd policy: {float(raw_cost)} > {spec.policy.max_cost_usd}"
                )
        return result
